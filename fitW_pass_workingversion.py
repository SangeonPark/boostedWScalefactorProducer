...
def drawFrame(iFrame,iData,iFuncs,iLegend,iColor,isPoisson=False):
    ...
    for pFunc in iFuncs:
        ...
        pFunc.plotOn(iFrame,r.RooFit.Name("DoubleCB"),r.RooFit.Components("*CB*"),r.RooFit.LineColor(5))
...
def drawDataMc(iVar,iData,iFuncsData,iMc,iFuncsMc,iRooFitResult_mc,iRooFitResult_data,params,iLabel='A'):
    ...
    lFrame.GetXaxis().SetTitle( "PUPPI Softdrop Jet Mass (GeV)")
    print 'adding entry'
    if 'pass' in iData.GetName():
        lLegend.AddEntry(lFrame.getCurve("model_total_mc_pT_200_100000_pass[Puppijet0_msd]"),"MC","l")
        lLegend.AddEntry(lFrame.getCurve("model_total_data_pT_200_100000_pass[Puppijet0_msd]"),"Data","l")
    if 'fail' in iData.GetName():
        lLegend.AddEntry(lFrame.getCurve("model_total_mc_pT_200_100000_fail[Puppijet0_msd]"),"MC","l")
        lLegend.AddEntry(lFrame.getCurve("model_total_data_pT_200_100000_fail[Puppijet0_msd]"),"Data","l")
    addInfo = getPavetext()
...
def getChi2NDOF(iData,iModel,iRooFitResult,iVar):
    ...
    print 'floating pars ',iRooFitResult.floatParsFinal().getSize(),' nbins ',iVar.getBins() 
    return pChi2.getVal(), int(2*iVar.getBins() - iRooFitResult.floatParsFinal().getSize())
